...
class Qiub_Slideshow_Adminhtml_SlideController extends Mage_Adminhtml_Controller_Action
{
	...
	public function indexAction() {
		$this->_initAction();       
		$this->_addContent($this->getLayout()->createBlock('slideshow/adminhtml_slides'));
		$this->renderLayout();
	}
	...
	public function editAction()
	{
		...
		$slidesModel  = Mage::getModel('slideshow/slides')->load($slideId);
		
		if ($slidesModel->getId() || $slideId == 0) {
 
			Mage::register('slideshow_data', $slidesModel);
			...
			$this->_addContent($this->getLayout()->createBlock('slideshow/adminhtml_slides_edit'))
				 ->_addLeft($this->getLayout()->createBlock('slideshow/adminhtml_slides_edit_tabs'));
	...
	public function saveAction()
	{
		if ( $this->getRequest()->getPost() ) {
			try {
				...
				$slidesModel = Mage::getModel('slideshow/slides');
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				...
				$slidesModel->setId($this->getRequest()->getParam('id'))
	...
	public function gridAction()
	{
		...
		$this->getResponse()->setBody(
			   $this->getLayout()->createBlock('slideshow/adminhtml_slides_grid')->toHtml()
		);
	}
}
?>